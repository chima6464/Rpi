...
//Defines Data Structure to register the modules as a character device 
static dev_t gpio_dev;
 
struct cdev gpio_cdev;

// defines a flag used to manage the access to the device driver in mutual exclusion: 
// only one user application is allowed to open the device file at a time;
static int gpio_lock = 0;

/* defines the data structure that contains the value read from the GPIO configured as input. 
As this data structure is written by an interrupt service routine, it shall be defined as volatile;*/
volatile char gpio_in_value = 0;

//ISR to read from GPIO configured as Input
...
// Virtual File System to access GPIO's
